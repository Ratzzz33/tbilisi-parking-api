...
Шаблон `deploy/tbilisi-parking-api.service` запускает API под systemd с
`Restart=always`: сервис стартует вместе с сервером и автоматически
перезапускается после сбоя.

`scripts/monitor.sh` предназначен для запуска раз в 5 минут. Он молчит при
нормальной работе, а при недоступности API или балансе ниже 10 GEL печатает
готовое уведомление. Его удобно подключать к cron, systemd timer или системе
уведомлений.
